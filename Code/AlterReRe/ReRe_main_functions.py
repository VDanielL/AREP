"""
The run_rere function is called by the main function.
This separation was necessary for being able to run the algorithm by other programs.
"""
...
def run_rere(test_instance, progress_for_executions, executions_max):
    ...
    if rere.OPERATION == 'manual':
        ...
        axs[0].plot_date(rere.data.timestamp, rere.data.value, fmt='-', label='original values')
    ...
    if rere.OPERATION == 'manual':
        axs[1].plot_date(rere.data.timestamp, rere.data.value, fmt='-', label='preprocessed data')
    ...
    while time < rere.length:
        ...
        # perform one timestep of the algorithm
